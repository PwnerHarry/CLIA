function [Z, SVM] = incremental_learn(Z, active_clusters_index, index_inactive_clusters, frontiers, SVM, Global)
...
status = check_status(size(Z, 1), active_clusters_index, index_inactive_clusters, Global);
if strcmp(status, 'unstable') || numel(frontiers) < 0.9 * MAX_ARCHIVE_SIZE
    return;
end
if numel(active_clusters_index) > Global.N && numel(active_clusters_index) == size(Z, 1)
    Z = generate_reference([], 0, Global, -1, 'normal');
elseif numel(active_clusters_index) < 0.95 * Global.N
    if MAX_REFERENCE_NUM_FLAG
        return;
    end
    [~, allocation] = pair([frontiers.objs], Z, 'sin');
    Z_active_old = Z(unique(allocation), :);
    [Z, ~] = generate_reference([frontiers.objs], numel(active_clusters_index), Global, 1, 'normal');
    if strcmp(normalization_str, 'normalize')
        if size(Z, 1) > 4 * Global.N
            normalization_str = '';
            [~, allocation] = pair([frontiers.objs], Z, 'sin');
            ...
            Z_next = reduce_useless(Z, delta, SVM, Global);
        else
            Z_next = Z;
        end
...
function Z = reduce_useless(Z, threshold, SVM, Global)
original_size = size(Z, 1);
if original_size <= Global.N
    ...
    elseif threshold > 1
        ...
        threshold = S(min(max(Global.N, threshold), numel(S)));
        index_remain = find(score >= threshold);
    end
    if numel(index_remain) < Global.N
        S = sort(score, 'descend');
        threshold = S(min(Global.N, numel(S)));
        index_remain = find(score >= threshold);
    end
end
Z = Z(index_remain, :);
...
function [status, index_i_active, index_i_inactive] = check_status(total_number, active_index, inactive_index, Global)
...
if status_initial_flag == true
    ...
    if pointer > 0 && stable_threshold(pointer) > 0
        ...
        threshold = min(20, max(5, ceil(Global.evaluation / 2e4)));
    end
end
if strcmp(normalization_str, 'normalize')
    fluctuation = 3;
